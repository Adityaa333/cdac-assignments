...
#include <linux/fs.h>
#include <linux/cdev.h> 	//class_creat() ,  device_creat()
#include <linux/device.h> 	//copy_to_user() , copy_from_user()
#include <linux/uaccess.h>	//kcalloc() ,  kfree()
...
static int __init multi_char_init(void){
	...
	if(ret < 0){
		pr_err("multi_char: failed to allocate number\n");
		return ret;
	}
...
#endif
	if(IS_ERR(multi_char_class)){ //check wether class creation failed
	...
	for(i = 0; i<DEVICE_COUNT; i++){//loop throught all minor devices
		...
		if(IS_ERR(device_create(multi_char_class, NULL, dev_num, NULL, "multi_char%d", i))){
